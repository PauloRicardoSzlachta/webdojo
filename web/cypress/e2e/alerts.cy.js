describe('Validações de Alertas em JavaScript', () => {
    ...
    it('Deve confirmar um diálogo e validar a respota positiva', () => {
        cy.on('window:confirm', (msg) => {
            expect(msg).to.equal('Aperte um botão!')
            return true; //True simula o clique no botão "OK" do alerta
        })
    ...
    it('Deve cancelar um diálogo e validar a respota negativa', () => {
        cy.on('window:confirm', (msg) => {
            expect(msg).to.equal('Aperte um botão!')
            return false; //False simula o clique no botão "Cancel" do alerta
        })
